# Hi there, I'm Atharv Tiwari! 👋

### A passionate coder and Final Year Computer Science student 🚀

I love building projects that solve real-world problems and bring creative ideas to life.  
Currently diving deeper into **Web Development**, **Cloud Computing**, and **System Design**.  
Always eager to learn, grow, and contribute to meaningful tech communities!

---

### 🚀 Skills and Technologies

- **Frontend:** React.js, TailwindCSS, HTML5, CSS3, JavaScript (ES6+)
- **Backend (Basic):** Node.js, Express.js (Learning in Progress)
- **Tools & Platforms:** Git, GitHub, Vercel, Netlify
- **Other Interests:** Cloud Computing, Three.js (Web 3D), DSA (Java)

---

### 📈 My Learning Journey (Currently Focused On)

- 🔥 Building real-world web projects
- 🔥 Deepening DSA knowledge
- 🔥 Preparing for top tech internships
- 🔥 Strengthening full-stack skills (MERN Stack learning in progress)

---

### 📫 Let's Connect

- [LinkedIn](https://www.linkedin.com/in/atharv-tiwari-08112a25b/)
- [Twitter](https://x.com/AtharvT19395104)
- [Portfolio (Coming Soon)](https://your-portfolio-link.com)

---

### 🛠️ Fun Fact

> "Consistency beats talent when talent doesn’t work hard."  
>  
> Always coding, always leveling up. 🚀

---

# 💻 My Recent Projects
- [E-Commerce Product Page (React + TailwindCSS)](https://nexusmart-chi.vercel.app/)
- [Upcoming: Portfolio Website (React + Three.js)](https://coming-soon-link.com)

---



